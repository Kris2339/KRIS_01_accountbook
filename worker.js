...
async function refreshSession(response, env) {
  ...
  refreshed.headers.set(
    "set-cookie",
    sessionCookie(await createSessionToken(env.SESSION_SECRET)),
  );
...
async function registerLoginFailure(env, addressHash, existing) {
  ...
  const isCurrentWindow =
    existing &&
    Number(existing.window_started_at) + LOGIN_WINDOW_SECONDS > nowSeconds;
  ...
  const windowStartedAt = isCurrentWindow
    ...
    : nowSeconds;
  const blockedUntil =
    failures >= MAX_LOGIN_FAILURES ? nowSeconds + LOGIN_WINDOW_SECONDS : null;
  ...
  await env.DB.prepare(
    ...
    .bind(addressHash, failures, windowStartedAt, blockedUntil)
    .run();
...
async function handleAuthApi(request, env, url) {
  ...
  await env.DB.prepare("DELETE FROM auth_attempts WHERE address_hash = ?")
    .bind(addressHash)
    .run();
...
async function readHousehold(env) {
  const [stateResult, transactionResult] = await env.DB.batch([
    env.DB.prepare(
...
async function saveHousehold(request, env) {
  if (request.headers.get("x-accountbook-protocol") !== "2") {
    return json(
      {
        error:
          "안전한 저장을 위해 앱을 다시 열어 새 버전을 사용해주세요. 기존 로컬 사본은 설정에서 내보내주세요.",
      },
      426,
    );
  }
  ...
  let body;
  try {
    // Enforce actual body bytes, including requests without Content-Length.
    const reader = request.body?.getReader();
    if (!reader) return json({ error: "요청 데이터가 없습니다." }, 400);
    ...
    while (true) {
      ...
      if (bytes > 2_000_000) {
        await reader.cancel();
        return json(
          { error: "저장할 데이터가 너무 큽니다. 내보내기로 보관해주세요." },
          413,
        );
      }
  ...
  const actor = String(body.clientId || "household").slice(0, 100);
  ...
  const settings = {
    ...
    monthlyBudgets: body.data.monthlyBudgets || {},
    lastSavedAt: now,
  };
  ...
  const statements = [
    ...
    ).bind(nextRevision, now, actor, body.data.transactions.length),
    env.DB.prepare(
      "UPDATE household_state SET revision = ?, initialized = 1, settings_json = ?, updated_at = ?, updated_by = ? WHERE id = 1 AND revision = ?",
    ).bind(nextRevision, JSON.stringify(settings), now, actor, baseRevision),
    env.DB.prepare(
      "UPDATE transactions SET deleted_at = ?, updated_at = ?, updated_by = ? WHERE deleted_at IS NULL AND id NOT IN (SELECT value FROM json_each(?))",
    ).bind(
      now,
      now,
      actor,
      JSON.stringify(body.data.transactions.map((t) => t.id)),
    ),
  ];
  statements.push(
    env.DB.prepare(
      `INSERT INTO transactions (id, transaction_date, data_json, updated_at, updated_by, deleted_at)
         SELECT json_extract(value,'$.id'), json_extract(value,'$.transactionDate'), value, ?, ?, NULL FROM json_each(?) WHERE 1
         ON CONFLICT(id) DO UPDATE SET
           ...
           updated_by = excluded.updated_by,
            deleted_at = NULL
         WHERE transactions.data_json <> excluded.data_json OR transactions.deleted_at IS NOT NULL`,
    ).bind(now, actor, JSON.stringify(body.data.transactions)),
  );
  ...
  if (url.pathname === "/api/data" && request.method === "GET") {
    if (request.headers.get("x-accountbook-protocol") !== "2") {
      return json(
        {
          error:
            "새 버전이 필요합니다. 앱을 닫고 다시 열어주세요. 기기 데이터는 삭제하지 마세요.",
        },
        426,
      );
    }
    ...
    try {
      ...
      if (!authenticated) {
        if (url.pathname === "/login.html")
          return fetchAsset(request, env, "/login.html", true);
